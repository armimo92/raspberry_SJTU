...
float thrust = 0;
float thrust_norm = 0;
int rc_CH_10 = 0;	//Channel for IBVS Activation ------> Offboard switch button
int rc_CH_8 = 0;	//Channel for points sending prior to Offboard switching------>Should be activated prior to CH 10 state change.
...
void rcInCallback(const mavros_msgs::RCIn::ConstPtr& msg){
    ...
    rc_CH_8 = msg->channels[7];
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "offb_switch_action");
    ros::NodeHandle nh;
    ros::Rate rate(100);
   ...
   ROS_INFO("Setpoints Sent!");
    ...
    while(ros::ok()){
	   ...
	   if( current_state.mode != "OFFBOARD" && (ros::Time::now() - last_request > ros::Duration(5.0)))
        ...
        if (rc_CH_10 < 1930 && current_state.mode == "OFFBOARD")
        {
          thrust = 8;
	...
	else if (rc_CH_10 > 1930 && current_state.mode == "OFFBOARD")
        {
