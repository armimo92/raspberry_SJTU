...
float aD = 0.000000205; //1.12 meters from VICON markers, 1.00 meter from target to camera
//float aD = 0.00000167; //1 meter
...
int main(int argc, char *argv[])
{
	...
	ros::Publisher flag_arucos_pub = nh.advertise<std_msgs::Bool>("flag_arucos",100);

    //image_transport::Subscriber sub = it.subscribe("/quad/camera/image_raw", 100, imageCallback); //Gazebo_camera 
    ros::Subscriber sub = nh.subscribe("raspicam_node/image/compressed", 100, &imageCallback);
	...
	std_msgs::Bool allArucosVisible;
    ...
    while (ros::ok())
	{
		...
		if (markerIds.size() != 4)
		{
            ...
            im_feat_vec.z = qz;
            im_feat_vec.w = qpsi;
	    allArucosVisible.data = flag_arucos;
			im_feat_pub.publish(im_feat_vec);
                        flag_arucos_pub.publish(allArucosVisible);
		}
